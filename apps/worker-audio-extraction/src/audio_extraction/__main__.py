"""Entry point for local HTTP and RunPod serverless: `python -m audio_extraction`."""
...
os.environ.setdefault("OMP_NUM_THREADS", "1" if os.environ.get("WHISPER_DEVICE", "cuda") == "cuda" else "4")
...
if os.environ.get("WHISPER_DEVICE", "cuda") != "cpu" and not cuda_smoke_test():
    logger.critical("CUDA is not functional — worker cannot process jobs. Exiting.")
    sys.exit(1)

mode = os.environ.get("WORKER_MODE", "runpod")
if mode == "http":
    from audio_extraction.http_worker import serve
    serve()
elif mode == "runpod":
    import runpod
    ...
    raise ValueError("WORKER_MODE must be http or runpod")
