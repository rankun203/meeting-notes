...
fn origin(input: &str) -> Result<String, String> {
    let url = reqwest::Url::parse(input).map_err(|_| "Enter a valid server URL")?;
    ...
    if !(url.scheme() == "https" || (url.scheme() == "http" && loopback))
        ...
        || url.path() != "/"
    {
        return Err("Use an HTTPS server origin (HTTP is allowed for localhost)".into());
    }
...
fn endpoint(discovery: &Value, key: &str, issuer_origin: &str) -> Result<String, String> {
    ...
    if parsed.origin().ascii_serialization() != issuer_origin {
        return Err("Provider endpoint does not belong to the server origin".into());
    }
...
impl GdayAuth {
    ...
    fn save(&self, session: &Option<Session>) -> Result<(), String> {
        ...
        result.map_err(|_| "Unable to save server credentials securely".into())
    }
    ...
    pub async fn begin(&self, base: &str, redirect: &str) -> Result<(String, String), String> {
        ...
        let discovery: Value = self
            ...
            .send()
            .await
            .map_err(|_| "Server discovery unavailable")?
            .error_for_status()
            .map_err(|_| "Server discovery unavailable")?
            .json()
            .await
        ...
        .await
        .map_err(|_| "Unable to verify server OIDC discovery")?;
        let registered: Value = self
            ...
            .error_for_status()
            .map_err(|_| "Server rejected public client registration")?
            .json()
            .await
    ...
    pub async fn complete(
        ...
        let token: CoreTokenResponse = self
            ...
            .error_for_status()
            .map_err(|_| "Server rejected sign-in code")?
            .json()
            .await
    ...
    pub async fn access_token(&self, expected_origin: &str) -> Result<String, String> {
        ...
        if session.expires_at <= chrono::Utc::now().timestamp() + 30 {
            let refresh = session
                .refresh_token
                .as_deref()
                .ok_or("Server sign-in expired; sign in again")?;
            let token: CoreTokenResponse = self
                .http
                ...
                .send()
                .await
                .map_err(|_| "Server token refresh unavailable")?
                .error_for_status()
                .map_err(|_| "Server sign-in expired or revoked; sign in again")?
                .json()
                .await
...
pub(crate) fn request_origin(headers: &HeaderMap) -> Result<String, String> {
    ...
    if headers.get("origin").and_then(|v| v.to_str().ok()) != Some(local.as_str()) {
        return Err("Server sign-in must start from this local app".into());
    }
    Ok(local)
}
