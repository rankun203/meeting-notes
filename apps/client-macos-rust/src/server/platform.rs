...
impl PlatformClient {
    ...
    pub async fn upload(&self, filename: &str, bytes: Vec<u8>) -> Result<String, String> {
        let value: Value = self
            ...
            .send()
            .await
            .map_err(|e| format!("Server audio upload failed: {e}"))?
            .error_for_status()
            .map_err(|e| format!("Server audio upload failed: {e}"))?
            .json()
            .await
            .map_err(|_| "Invalid server upload response")?;
        let url = value["url"]
            .as_str()
            .ok_or("Server upload response has no URL")?;
    ...
    pub async fn upload_file(
        ...
        if !response.status().is_success() {
            return Err(format!("Server rejected {filename} ({})", response.status()));
        }
    ...
    pub async fn imported_meeting(&self, external_id: &str) -> Result<Option<Value>, String> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|_| "Unable to verify server meeting; retry migration")?;
        if response.status() == reqwest::StatusCode::NOT_FOUND {
            return Ok(None);
        }
        if !response.status().is_success() {
            return Err(format!("Server import verification failed ({}); check server version, login or conflicting meeting", response.status()));
        }
        response
            .json()
            .await
    ...
    pub async fn import_meeting(&self, body: &Value) -> Result<Value, String> {
        ...
        if !response.status().is_success() {
            return Err(format!(
                "Server rejected meeting import ({}); no local files were removed",
                response.status()
            ));
        }
        response
    ...
    pub async fn execute(
        ...
        for attempt in 0..4 {
            ...
            match response {
                ...
                {
                    return Err(format!(
                        "Server rejected transcription task ({})",
                        response.status()
                    ));
                }
                _ if attempt == 3 => {
                    return Err(
                        "Server task submission unavailable; retry transcription to recover the same task".into()
                    );
                }
    ...
    pub async fn ensure_transcription_available(&self) -> Result<(), String> {
        ...
        if value["transcription"] != true {
            return Err("Server transcription is not configured; ask its administrator to configure the worker".into());
        }
    ...
    pub async fn task_result(&self, task_id: &str) -> Result<TaskResult, String> {
        let value: Value = self
            ...
            .send()
            .await
            .map_err(|_| "Server task lookup unavailable; it can be resumed later")?
            .error_for_status()
            .map_err(|_| "Server task lookup failed; check your sign-in")?
            .json()
            .await
            .map_err(|_| "Invalid server task response")?;
