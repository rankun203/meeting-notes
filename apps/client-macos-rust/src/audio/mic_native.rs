...
mod platform {
    ...
    /// System Settings > Sound > Input. Only constructed in the capture helper;
    /// configuration changes make that process exit so the parent can retry
    /// with a fresh engine without invoking a potentially wedged native stop.
