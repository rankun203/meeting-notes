...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            // Apply macOS vibrancy (NSVisualEffectView Sidebar material) so the
            // window chrome blends with the OS — matches the 1Password 8 look.
            #[cfg(target_os = "macos")]
            {
            ...
            // Bootstrap AppState (mirrors meeting-notes-daemon's src/main.rs).
            ...
            tauri::async_runtime::spawn(async move {
                ...
                let shared_secrets =
                    std::sync::Arc::new(tokio::sync::RwLock::new(llm_secrets));
                ...
                // Regenerate markdown indexes so Claude Code integrations stay in sync.
                {
                    ...
                    meeting_notes_daemon::markdown::write_claude_md(
                        &data_dir,
                        self_intro.as_deref(),
                    );
                }
