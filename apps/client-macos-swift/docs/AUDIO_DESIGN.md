...
Voice processing can reduce other apps' playback volume. Configure minimum ducking and leave advanced ducking off to minimize interference. New recordings enable processing when the default output’s Core Audio stream terminal type identifies a speaker or low-frequency speaker. Headphones, line outputs, digital interfaces, and unknown or unreadable routes default to unprocessed capture. The New Recording override applies only to that session; legacy saved processing preferences are ignored. The route is checked again after microphone permission is granted, before capture creates aggregate devices, unless the user explicitly overrides it. This detects the Mac’s default output, not an unrelated calling app’s independently selected output. Apple's documentation describes voice-processing capabilities, but it does not establish reliable cancellation of every unrelated app's speaker output across every device route. Do not label the result “echo-free.” Headphones avoid the speaker-to-microphone acoustic path; speakerphone quality requires real hardware testing. Voice Isolation is a system-controlled mic mode, not a second custom denoiser to stack blindly on top. [WWDC23 voice processing and ducking](https://developer.apple.com/videos/play/wwdc2023/10235/).
