...
- Give icon buttons a full-area hit shape, visible hover/pressed feedback, and a tooltip. Use at least 44×44 pt for player controls (including menu triggers and track mute buttons), and 20 pt height with horizontal padding for preview sample drag sources (an explicit compact developer-bar exception); keep symbols smaller inside the target. Apple's macOS accessibility table lists 28×28 pt default and 20×20 pt minimum, while general Buttons guidance recommends 44×44 pt hit regions. The minimum is not our target for frequently used controls. Hover feedback must not resize or move controls.
