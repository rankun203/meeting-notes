...
final class TimedAudioWriter {
    ...
    init(url: URL, format: AVAudioFormat, epoch: TimeInterval, voiceProcessed: Bool = false) throws {
        ...
        profile = AudioTrackProfile(
            filename: url.lastPathComponent, sampleRate: format.sampleRate, channels: format.channelCount,
        ...
        do {
            guard buffer.format == format else {
                throw MeetingError.message(
                    "The audio format changed. The partial recording was saved; start a new recording with the new device."
                )
            }
            ...
            if gap > Int64(format.sampleRate * 0.002) {
                guard gap <= Int64(format.sampleRate * 30) else {
                    throw MeetingError.message(
                        "Audio capture was interrupted for over 30 seconds. The partial recording was saved.")
                }
                while gap > 0 {
