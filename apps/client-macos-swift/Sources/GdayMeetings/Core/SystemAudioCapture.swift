...
final class SystemAudioCapture: @unchecked Sendable {
    ...
    func start() throws {
        do {
            ...
            try check(AudioObjectGetPropertyData(AudioObjectID(kAudioObjectSystemObject), &address, UInt32(MemoryLayout<pid_t>.size), &pid, &size, &process), "Identify this app's audio process")
            guard process != kAudioObjectUnknown else { throw MeetingError.message("Core Audio could not identify this app for exclusion from system audio. Try recording again.") }
            ...
            guard originalFormat.mFormatID == kAudioFormatLinearPCM,
                  ...
                  originalFormat.mSampleRate > 0 else {
                throw MeetingError.message("The system-audio tap returned an unsupported format. Select a standard stereo output device and try again.")
            }
            let interleaved = flags & kAudioFormatFlagIsNonInterleaved == 0
            guard originalFormat.mBytesPerFrame == (interleaved ? 8 : 4),
                  let outputFormat = AVAudioFormat(commonFormat: .pcmFormatFloat32, sampleRate: originalFormat.mSampleRate, channels: 2, interleaved: true),
                  let outputBuffer = AVAudioPCMBuffer(pcmFormat: outputFormat, frameCapacity: maxFrames),
                  let ring = GdayAudioRingCreate(2, interleaved, maxFrames, 64) else {
                throw MeetingError.message("Could not allocate the bounded system-audio capture buffer.")
            }
            self.ring = ring; format = outputFormat; buffer = outputBuffer
            var uidReference: Unmanaged<CFString>?
            size = UInt32(MemoryLayout<Unmanaged<CFString>?>.size)
            address.mSelector = kAudioTapPropertyUID
            try check(AudioObjectGetPropertyData(tap, &address, 0, nil, &size, &uidReference), "Read system-audio tap identity")
            ...
            try check(AudioHardwareCreateAggregateDevice(composition as CFDictionary, &aggregate), "Create private audio capture device")
            try check(AudioDeviceCreateIOProcID(aggregate, GdayAudioIOProc, UnsafeMutableRawPointer(ring), &ioProc), "Prepare audio capture callback")
    ...
    private func observe(_ object: AudioObjectID, selector: AudioObjectPropertySelector) throws {
        ...
        let block: AudioObjectPropertyListenerBlock = { [weak self] _, _ in
            self?.fail(MeetingError.message("The system-audio device or format changed. Available audio will be saved; start a new recording on the current device."))
        }
    ...
    private func drain() {
        guard let ring, let buffer, let destination = buffer.mutableAudioBufferList.pointee.mBuffers.mData else { return }
        var frames: UInt32 = 0; var hostTime: UInt64 = 0
        // Bound each consumer pass as well; stop runs after hardware delivery ends.
        for _ in 0..<64 {
            guard GdayAudioRingRead(ring, destination.assumingMemoryBound(to: Float.self), maxFrames, &frames, &hostTime) else { break }
        ...
        case 0: break
        case 1: fail(MeetingError.message("System-audio capture could not keep up with incoming audio. Available audio will be saved; close busy applications before recording again."))
        case 3: fail(MeetingError.message("System audio lost its host-clock timestamp. Available audio will be saved."))
        default: fail(MeetingError.message("The system-audio buffer format changed or exceeded its supported size. Available audio will be saved."))
        }
    }
    private func fail(_ error: Error) {
        guard !failed else { return }; failed = true; onFailure?(error)
    }
    func stop() throws {
        ...
        var first: Error?
        func remember(_ status: OSStatus, _ operation: String) { if status != noErr && first == nil { first = MeetingError.message("\(operation) failed (Core Audio \(status)).") } }
        for (object, var address, block) in listeners { remember(AudioObjectRemovePropertyListenerBlock(object, &address, queue, block), "Remove audio observer") }
        listeners.removeAll()
        if running { remember(AudioDeviceStop(aggregate, ioProc), "Stop system audio"); running = false }
        var callbackUnregistered = true
        if let ioProc {
            let status = AudioDeviceDestroyIOProcID(aggregate, ioProc)
            callbackUnregistered = status == noErr
            remember(status, "Remove audio callback"); self.ioProc = nil
        }
        timer?.cancel(); timer = nil
        queue.sync { drain(); onBuffer = nil; onFailure = nil }
        if aggregate != kAudioObjectUnknown { remember(AudioHardwareDestroyAggregateDevice(aggregate), "Remove capture device"); aggregate = kAudioObjectUnknown }
        if tap != kAudioObjectUnknown { remember(AudioHardwareDestroyProcessTap(tap), "Remove audio tap"); tap = kAudioObjectUnknown }
        queue.sync {
            ...
            self.ring = nil; buffer = nil
        }
        if let first { throw first }
    }
    private func check(_ status: OSStatus, _ operation: String) throws {
        guard status == noErr else {
            throw MeetingError.message("\(operation) failed (Core Audio \(status)). If macOS denied audio recording, allow Gday Meetings in System Settings → Privacy & Security → Screen & System Audio Recording, then try again. No screen recording is requested.")
        }
    }
}
