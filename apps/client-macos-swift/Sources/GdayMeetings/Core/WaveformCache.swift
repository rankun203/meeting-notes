...
struct WaveformCache: Sendable {
    ...
    private struct Identity: Codable, Equatable {
        ...
        init(_ url: URL) throws {
            ...
            self.created = values[.creationDate] as? Date
        }
    }
    ...
    func entryURL(for source: URL) -> URL {
        let key = SHA256.hash(data: Data(source.standardizedFileURL.path.utf8)).map { String(format: "%02x", $0) }
            .joined()
    ...
    private func load(_ source: URL) -> AudioWaveform? {
        guard let identity = try? Identity(source),
            ...
            entry.waveform.peaks.allSatisfy({ $0.isFinite && $0 >= 0 && $0 <= 1 })
        else { return nil }
    ...
    func waveform(source: URL, readable: URL) async throws -> AudioWaveform {
        ...
        await Task.detached(priority: .utility) {
            do {
                ...
                // A disposable cache failure must not prevent playback.
            }
        }.value
        return waveform
    }
