...
enum AudioPlaybackPreparation {
    ...
    static func opusMetadata(_ source: URL) async throws -> (channels: Int, duration: Double) {
        ...
        guard let final, final >= max(Int64(reader.preSkip), frames - lastPacketFrames), final <= frames, reader.sawEnd else { throw ServiceError("The Opus recording has no valid final duration.") }
    ...
    private static func decodeOpus(_ source: URL, to destination: URL) throws {
        let reader = try OggOpusReader(source)
        var description = AudioStreamBasicDescription(mSampleRate: 48000, mFormatID: kAudioFormatOpus, mFormatFlags: 0, mBytesPerPacket: 0, mFramesPerPacket: 0, mBytesPerFrame: 0, mChannelsPerFrame: UInt32(reader.channels), mBitsPerChannel: 0, mReserved: 0)
        guard let compressed = AVAudioFormat(streamDescription: &description),
              ...
              let output = AVAudioPCMBuffer(pcmFormat: pcm, frameCapacity: 5760) else { throw ServiceError("The native Opus decoder is unavailable on this Mac.") }
        ...
        while let packet = try reader.nextAudioPacket() {
            try Task.checkCancellation()
            let input = AVAudioCompressedBuffer(format: compressed, packetCapacity: 1, maximumPacketSize: max(1, packet.data.count))
            input.byteLength = UInt32(packet.data.count); input.packetCount = 1
            packet.data.copyBytes(to: input.data.assumingMemoryBound(to: UInt8.self), count: packet.data.count)
            input.packetDescriptions?.pointee = AudioStreamPacketDescription(mStartOffset: 0, mVariableFramesInPacket: UInt32(try opusPacketFrames(packet.data)), mDataByteSize: UInt32(packet.data.count))
            var supplied = false
            var error: NSError?
            ...
            if let granule = packet.finalGranule {
                guard granule >= Int64(reader.preSkip) + written, granule <= decoded else { throw ServiceError("The Opus final duration lies outside its final decoded packet.") }
    ...
    private static func opusPacketFrames(_ packet: Data) throws -> Int {
        ...
        else { frame = config & 3 == 3 ? 2880 : 480 << (config & 3) }
        let count: Int
        switch toc & 3 {
        case 0: count = 1
        case 1, 2: count = 2
...
private final class OggOpusReader {
    struct Packet { let data: Data; let finalGranule: Int64? }
    ...
    init(_ url: URL) throws {
        ...
        guard let identification = try nextPacket()?.data, identification.count >= 19,
              ...
              [1, 2].contains(identification[9]), identification[18] == 0 else {
        ...
        guard let tags = try nextPacket()?.data, tags.prefix(8) == Data("OpusTags".utf8) else { throw ServiceError("The Opus comment header is missing.") }
    }
    ...
    private func readExactly(_ count: Int) throws -> Data {
        ...
        while result.count < count {
            guard let chunk = try file.read(upToCount: count - result.count), !chunk.isEmpty else { throw ServiceError("The Ogg recording is truncated.") }
            result.append(chunk)
        }
        return result
    }
    private func readPage() throws {
        ...
        guard header.prefix(4) == Data("OggS".utf8), header[4] == 0 else { throw ServiceError("Unsupported Ogg audio container.") }
        ...
        var crc: UInt32 = 0
        for byte in page { crc ^= UInt32(byte) << 24; for _ in 0..<8 { crc = crc & 0x80000000 != 0 ? (crc << 1) ^ 0x04c11db7 : crc << 1 } }
        ...
        if serial == nil { serial = pageSerial; guard header[5] & 2 != 0 else { throw ServiceError("Missing Ogg stream beginning.") } }
        guard serial == pageSerial, Self.uint32(header, 18) == sequence else { throw ServiceError("Chained or interrupted Ogg streams are not supported.") }
        sequence &+= 1
        ...
        for (index, lace) in laces.enumerated() {
            let count = Int(lace)
            partial.append(body[cursor..<(cursor + count)]); cursor += count
            ...
            if count < 255 {
                ...
                packets.append(Packet(data: partial, finalGranule: final)); partial.removeAll(keepingCapacity: true)
            }
        }
        if ended {
            guard partial.isEmpty else { throw ServiceError("The final Ogg packet is incomplete.") }
            if let trailing = try file.read(upToCount: 1), !trailing.isEmpty { throw ServiceError("Chained Ogg streams or trailing bytes are not supported.") }
            sawEnd = true
        }
    }
