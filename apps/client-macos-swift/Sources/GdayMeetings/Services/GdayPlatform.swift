...
enum ServerTaskResult { case pending; case complete([ServerTranscriptSegment]); case failed(String) }
...
extension GdayServerService {
    func ensureTranscriptionAvailable() async throws {
        ...
        guard result["durableTasks"] as? Bool == true, result["transcription"] as? Bool == true else { throw ServiceError("This server does not have durable transcription configured. Ask its administrator to configure a worker.") }
    }
    func upload(file: URL) async throws -> URL {
        let size = try file.resourceValues(forKeys: [.fileSizeKey]).fileSize ?? 0
        guard size > 0, size <= 500_000_000 else { throw ServiceError("Audio must be nonempty and no larger than 500 MB. Use M4A, Opus, or MP3 for long recordings.") }
    ...
    func submit(externalID: String, title: String, inputs: [ServerTrackInput], language: String, diarize: Bool, idempotencyKey: String) async throws -> String {
        guard !idempotencyKey.isEmpty else { throw ServiceError("A durable transcription attempt requires an idempotency key.") }
        ...
        r.httpBody = try JSONSerialization.data(withJSONObject: ["externalId": externalID, "title": title, "inputs": inputs.map { ["url": $0.url.absoluteString, "trackName": $0.trackName, "sourceType": $0.sourceType, "channels": $0.channels] as [String: Any] }, "executionOptions": ["language": language, "diarize": diarize], "idempotencyKey": idempotencyKey])
        let result = try await ServiceHTTP.json(r)
        guard let id = result["id"] as? String else { throw ServiceError("The server returned no task ID. Retry using the saved attempt to recover the same task.") }
        return id
    }
    ...
    static func parseTask(_ result: [String: Any]) throws -> ServerTaskResult {
        guard let outputs = result["outputs"] as? [[String: Any]] else { throw ServiceError("The server task has no outputs list.") }
        if let output = outputs.last(where: { $0["type"] as? String == "TRANSCRIPT_OUTPUT" }) {
            guard let body = output["body"] as? [String: Any], let tracks = body["tracks"] as? [String: [String: Any]] else { throw ServiceError("The server returned an invalid transcript.") }
            var segments: [ServerTranscriptSegment] = []
            for (name, track) in tracks {
                guard let entries = track["segments"] as? [[String: Any]] else { throw ServiceError("The transcript track is missing its segments.") }
                for entry in entries {
                    guard let start = entry["start"] as? Double, let end = entry["end"] as? Double, let text = entry["text"] as? String, start.isFinite, end.isFinite, start >= 0, end >= start else { throw ServiceError("The server transcript contains an invalid segment.") }
                    segments.append(.init(start: start, end: end, text: text, speaker: entry["speaker"] as? String, track: name))
                }
            }
            return .complete(segments.sorted { $0.start == $1.start ? $0.track < $1.track : $0.start < $1.start })
        }
        let status = result["status"] as? String ?? ""
        if ["FAILED", "CANCELLED", "TIMED_OUT"].contains(status) { return .failed("Transcription \(status.lowercased().replacingOccurrences(of: "_", with: " ")).") }
    ...
    func search(query: String) async throws -> [ServerMeeting] {
        ...
        let result = try await ServiceHTTP.json(r)
        guard let meetings = result["meetings"] as? [[String: Any]] else { throw ServiceError("Invalid server search results.") }
        return meetings.compactMap { value in
            guard let id = value["id"] as? String, let title = value["title"] as? String else { return nil }
            return ServerMeeting(id: id, externalID: value["externalId"] as? String ?? id, title: title, transcript: value["transcript"] as? String ?? "")
        }
    }
    func ensureArchiveAvailable() async throws {
        ...
        guard capabilities["meetingImports"] as? Bool == true else { throw ServiceError("Upgrade the server to support existing-meeting imports.") }
    }
    ...
    func verifyArchive(externalID: String) async throws -> [String: Any] {
        var r = try await authorizedRequest("api/platform/meetings/import")
        r.url = r.url!.appendingPathComponent(externalID); r.timeoutInterval = 900
