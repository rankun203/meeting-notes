...
New Recording enables **Microphone Voice Processing** by default when the Mac’s default output reports a speaker. Headphones and unidentified routes default to unprocessed audio. You can override this choice for each recording under Recording options; it is not saved in Settings. Processing uses Apple's echo cancellation, noise suppression, and automatic gain control, and can affect other apps' playback volume. The implementation requests minimum ducking, never monitors the microphone through speakers, and saves system audio separately. Echo removal depends on the device route; headphones provide the most reliable acoustic separation. Voice processing cannot guarantee echo-free recordings from every third-party calling app.
