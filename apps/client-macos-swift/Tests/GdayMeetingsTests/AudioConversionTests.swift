...
struct AudioConversionTests {
    @Test(arguments: [1, 2]) func aacConversionPreservesDurationAndChannels(channels: Int) async throws {
        ...
        let format = try #require(
            AVAudioFormat(standardFormatWithSampleRate: 48000, channels: AVAudioChannelCount(channels)))
        ...
        for channel in 0..<channels {
            ...
            for frame in 0..<48000 {
                samples[frame] = Float(sin(Double(frame) * 2 * .pi * Double(440 + 220 * channel) / 48000) * 0.1)
            }
        }
        ...
        let decoded = try #require(
            AVAudioPCMBuffer(pcmFormat: encoded.processingFormat, frameCapacity: AVAudioFrameCount(encoded.length)))
