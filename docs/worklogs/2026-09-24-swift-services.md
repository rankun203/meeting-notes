...
## In-app system-audio consent

The user reported ScreenCaptureKit `userDeclined` before seeing consent and required authorization through the app. The SDK 27 `CGWindow.h` documents that global screen-capture permission requests do not re-prompt a previously denied process. Apple recommends `SCContentSharingPicker`; its explicit content selection authorizes the current session without a separate global Screen Recording grant ([WWDC23 privacy](https://developer.apple.com/videos/play/wwdc2023/10053/)).

Reworked `Core/RecordingPermissions.swift` and `AudioCapture.swift` to request microphone access, then present the native single-display picker before starting any audio engine or stream. Capture uses the returned `SCContentFilter` directly; it no longer enumerates `SCShareableContent` or gates on global Core Graphics permission. A main-actor observer remains alive until stream stop, disallows content changes and unregisters on failure/stop. Cancellation resumes the pending selection exactly once; `cancelPendingSelection()` lets the store unblock Quit while the picker is waiting. ScreenCaptureKit still receives only an audio output; no video is saved.

This is session-scoped consent rather than a TCC/signature workaround. No privacy database, permission reset, signing requirement or persistent-capture entitlement was changed. Picker verification and the final integrated suite are coordinated by root. Remaining limitation: a macOS microphone authorization dialog is managed by AVFoundation and is separate from the cancellable system-audio selection.
