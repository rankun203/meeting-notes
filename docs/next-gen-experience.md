...
- **Act from the toolbar.** Export uses an outlined download button and a format picker with descriptions and file extensions. Regenerate uses a sage button with a refresh icon, collapsing to an accessible icon button on phones. Escape dismisses the export picker and returns focus to its trigger. [Export picker preview](images/next-gen-export.png).
